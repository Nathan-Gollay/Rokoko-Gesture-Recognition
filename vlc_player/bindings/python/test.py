...
try:
    from pycocoa import __name__    as _pycocoa_, __version__ as _pycocoa_version
except ImportError:
...
#_AppleSi = machine().startswith('x86_64')
...
#       self.media   = None
       ...
       # self.viewpoint = vlc.VideoViewpoint(pitch =)
        self.viewpoint.yaw = 0
        self.viewpoint.roll = 0
        self.viewpoint.pitch = 0
        self.viewpoint.fov = 70
        self.viewpoint.zoom = 3
        
        self.shutdown = shutdown
        self.pipe = pipe
        self.pipe2 = pipe2
        #vlc.video_viewpoint_set_yaw(self.viewpoint, 90)
...
#           self.player = i.media_player_new()
    ...
    def roll(self):
        i = float(0.0)
        while True:
#            self.viewpoint.contents.pitch = i
            #self.viewpoint.contents.roll = i
            self.viewpoint.contents.yaw = i
	    #self.viewpoint.contents.zoom = .1
            #self.viewpoint.contents.fov = 90
            if i == 90.0:
                i = 0.0
            i +=.01
            time.sleep(.03)
    ...
    def appLaunched_(self, app):
        ...
        self.window = MediaWindow(title=self.video or self.title, fraction =1)
        ...
        if self.player and self.video and isfile(self.video):
           ...
           # print(self.player.video_update_viewpoint(self.viewpoint,True))
...
#       self._saturation(item)
    ...
    def menuPlay_(self, item_or_None):  # PYCHOK expected
        ...
        #self.viewpoint.contents.pitch = 0
        #self.viewpoint.contents.fov = 100
        #self.viewpoint.contents.zoom = 1
        
        self.player.play()
        time.sleep(1)
        #self.window.setFullScreen_(True)
        #self.sampleViewpoint()
    
        #self.viewpoint.pitch = 90
       
        self.player.video_update_viewpoint(self.viewpoint,False)
        yaw_thread = threading.Thread(target=self.sampleViewpoint)
        yaw_thread.start()
...
def start(pipe, pipe2, video, shutdown):
    def _Adjustr():
    ...
    while not received:
        received = pipe.recv()
        print(received, "fuck ya ")
    #args = sys.argv[1:]
    #pipe_child_conn = args.pop(0)
    #print(pipe_child_conn)
    #pipe_fd = int(pipe_child_conn)
    #pipe_connection = os.fdopen(pipe_fd, 'r')
