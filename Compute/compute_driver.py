...
def runSignLanguage(skeleton, record_button_pushed, recording_name_conn):
    ...
    #signs.record(skeleton, letters = ['1', '2'])
...
def videoDragDrop2D(skeleton, child_conn, record_button_pushed, save_button_pushed, load_button_pushed, 
        pose_name_conn, identifier_conn, video_parent_conn, video_parent_conn_2, skeleton_ready, 
        shutdown, sensitivity_slider, recording_name_conn, bone_name_conn, left_end = None, right_end = None, up_end = None, down_end = None):
    
    #left_end, right_end, up_end, down_end = determineRotationAxis(skeleton)
    left_end, right_end, up_end, down_end = loadCalibrationFromCSV()
    lock = multiprocessing.Lock()
    while True:
        time.sleep(2)
        #left_end, right_end = determine_rotation_axis(skeleton)
            ...
            if run:
                ...
                if is_closed and was_closed:
                    ...
                    if new_yaw != None and old_yaw != None and new_pitch != None and old_pitch != None:
                        ...
                        #print("send")
                        xy_queue.append((old_yaw - new_yaw, old_pitch - new_pitch))
                        new_yaw_change, new_pitch_change = simpleMovingAverage(list(xy_queue), 7)
                    ...
                    new_yaw = None
                    new_pitch = None
                    while new_yaw == None or new_pitch == None:
                        new_yaw = axisDistance(left_end, right_end, skeleton)
                        new_pitch = axisDistance(up_end, down_end, skeleton)
    ...
    while True:
        ...
        if is_pointer and not was_click:
            ...
            if x != None and y != None and old_x != None and old_y != None:
                ...
                x, y = simpleMovingAverage(list(position_queue), 5)
...
def mouseAndSignLanguage(skeleton, sensitivity_slider, record_button_pushed, recording_name_conn):    
    ...
    keyboard_on = Value('i', 0)

    alphabet = string.ascii_uppercase
    signs = SignLanguage("Nathan", record_button_pushed, recording_name_conn)
    #signs.record(skeleton, letters = ['1'])
    signs.loadRecordings(letters = ['_', '-', '+', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I','K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y',  '1', '2'])
    sign_thread = Thread(target=signs.runAverageAngleComparison, args =(skeleton, keyboard_on, True))
